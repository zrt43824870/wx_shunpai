...
Page({
  ...
  */
  ...
  SubOrder: function (e) {
    ...
    if (!this.data.addressId) return app.Tips({ title: '请选择联系人信息' });
    data = {
      ...
      mark: that.data.mark,
      daikuan:that.data.daikuan,
      banzheng:that.data.banzheng,
      zhidao:that.data.zhidao
    };
    // if (data.payType == 'yue' && parseFloat(that.data.userInfo.now_money) < parseFloat(that.data.totalPrice)) return app.Tips({ title: '余额不足！' });
    wx.showLoading({ title: '预约看样中' });
    // console.log(this.data);
    app.basePost(app.U({ c: 'auth_api', a: 'create_book', q: { key: this.data.orderKey } }), data, function (res) {
